...
const EpicaForm = ({ epica, proyectoId, onSubmit, onCancel }) => {
  ...
  // Cargar datos de la épica si estamos editando
  useEffect(() => {
    if (epica) {
      setFormData({
        titulo: epica.titulo || "",
        descripcion: epica.descripcion || "",
        proyecto: epica.proyecto || proyectoId || "",
        prioridad: epica.prioridad || "media",
        estado: epica.estado || "pendiente",
        fechaInicio: epica.fechaInicio
          ? new Date(epica.fechaInicio).toISOString().split("T")[0]
          : new Date().toISOString().split("T")[0],
        fechaFin: epica.fechaFin ? new Date(epica.fechaFin).toISOString().split("T")[0] : "",
      })
    }
  }, [epica, proyectoId])
  ...
  const validateForm = () => {
    ...
    if (!formData.titulo.trim()) {
      errors.titulo = "El título de la épica es obligatorio"
    }
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      setError(
        err.response?.data?.message || "Error al guardar la épica. Por favor, verifica los datos e intenta de nuevo.",
      )
    } finally {
      setLoading(false)
    }
    ...
    <form onSubmit={handleSubmit} className="space-y-4">
      ...
      <div className="flex justify-end space-x-3 mt-6">
        ...
        <button
          ...
          disabled={loading}
        >
          {loading ? "Guardando..." : epica ? "Actualizar Épica" : "Crear Épica"}
        </button>
      </div>
    </form>
  )
